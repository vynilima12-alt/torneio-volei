...
with aba_confronto:
    ...
    if jugadores_a.empty or jugadores_b.empty:
        ...
        with m1:
            st.markdown(f"<h3 style='text-align: right;'>{time_a_sel}</h3>", unsafe_allow_html=True)
            st.markdown(f"<h1 style='text-align: right; color: green;'>{st.session_state.sets_ganhos_a} <span style='font-size:20px; color:gray;'>sets</span></h1>", unsafe_allow_html=True)
        with m2:
            st.markdown("<h1 style='text-align: center; color: gray;'>VS</h1>", unsafe_allow_html=True)
            st.markdown(f"<p style='text-align: center;'><b>Set {st.session_state.set_atual}</b><br>{st.session_state.placar_set_a} x {st.session_state.placar_set_b}</p>", unsafe_allow_html=True)
        with m3:
            st.markdown(f"<h3 style='text-align: left;'>{time_b_sel}</h3>", unsafe_allow_html=True)
            st.markdown(f"<h1 style='text-align: left; color: green;'>{st.session_state.sets_ganhos_b} <span style='font-size:20px; color:gray;'>sets</span></h1>", unsafe_allow_html=True)
